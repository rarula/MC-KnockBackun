...
#region 通常計算結果をMotionに代入
        execute if entity @s[tag=!CAN_KBR] store result entity @s Motion[1] double 0.0001 run scoreboard players get *KnockBack.Y* KnockBack.Calc_c
#endregion

#region 追加計算結果をMotionに代入
    # Attribute
        execute if entity @s[tag=CAN_KBR,tag=CAN_KBR_ATT] store result entity @s Motion[1] double 0.01 run scoreboard players get *KnockBack.Y* KnockBack.Calc_c
#endregion